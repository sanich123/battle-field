...
import { TYPES_SHIP } from './const.js';
...
export function killAroundPositions(killedPositions: ShipPosition['allPositions']) {
  ...
  if (isHorizontal) {
    ...
    const realMinX = minX > 0 ? minX - 1 : minX;
    const realMaxX = maxX < 9 ? maxX + 1 : maxX;
    ...
    const realMinY = minY > 0 ? minY - 1 : minY;
    const realMaxY = maxY < 9 ? maxY + 1 : maxY;
